...
version: v1.0.0
phase: v0.6
applies_to: 04_reviews/weekly/YYYY-Www.md · 04_reviews/monthly/YYYY-MM.md · 04_reviews/milestones.md
source_of_truth:
...
2. **异常预警强制**：检测到连续 ≥3 天异常模式（饮食量偏少/睡眠质量差/情绪烦躁或哭闹/同一健康异常持续）→ 预警条目置顶 + 护理调整建议 + 就医指征提示
3. **红旗处理**：日志或对照中出现 `milestones.json` 的 red_flags（含 universal_red_flags「技能倒退」）→ 显著提示「请及时儿保科评估」+ 免责声明，**不做诊断**
4. **里程碑对照必做**：周/月复盘必须包含四域里程碑对照（读 `milestones.json` 当前与下一月龄段），更新 `04_reviews/milestones.md` 时间轴
5. **建议可执行可转化**：优化建议 3-5 条，每条含具体动作 + 依据数据 + 「☐ 转化为规则」标记；家长确认后调用 kid-rules 注入（source 标注复盘来源）
6. **不评判措辞**：报告用语中立（「本周蔬菜摄入种类 4 种，低于建议的 5 种」✔；「你们对孩子饮食不够上心」✘）
...
1. 周期内全部 `01_journal/` 日志 + `02_plans/` 方案
2. `tags.md` 当前状态；rules.md 变更（如有）
3. 上期复盘（环比趋势用）
4. 统计数据覆盖率：有日志天数 / 周期总天数（<50% 时输出「数据不足」警告，结论降级为「初步观察」）
...
3. 下一月龄段重点预览（「未来 3 个月发展重点」）
4. 更新 `04_reviews/milestones.md` 时间轴（增量，保留历史判断与变更记录）
...
扫描周期内日志序列：
  饮食量=偏少 连续≥3天 → 预警
  睡眠质量=差 连续≥3天 → 预警
  情绪∈{fussy,crying} 连续≥3天 → 预警
  同一 illness_notes 持续≥3天 → 预警
```
...
| 项 | 值 |
|----|----|
| 周报 | `04_reviews/weekly/YYYY-Www.md` |
| 月报 | `04_reviews/monthly/YYYY-MM.md` |
| 时间轴 | `04_reviews/milestones.md`（增量更新） |
