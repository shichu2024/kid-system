...
| 类别 | 指令 |
|------|------|
| 基础管理 | `初始化档案` `查看孩子档案` `添加过敏` `更新偏好` `切换孩子` |
| 知识规则 | `注入规则` `查看已注入规则` `调整优先级` `停用规则` |
| 日志记录 | `记录今天` `记录饮食/运动/学习` `查看昨天的日志` `搜索xx记录` |
| 方案生成 | `今日方案` `今日菜单` `今日学习计划` `今日游戏推荐` `生成本周菜单` `未来3天计划` |
| 方案调整 | `把午餐换成清淡的` `增加户外游戏` `替换食材` |
| 复盘分析 | `周总结` `月度报告` `查看发育里程碑` `下一阶段重点` |
